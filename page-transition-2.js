...
// Запускаем проверку CSS при загрузке
setTimeout(() => {
  checkCSSTransitions()
}, 1000)
...
// Инициализация Barba с улучшенной логикой hover и проверкой видимости
if (isDesktop()) {
  setupBarbaHooks()
            ...
            if (activeItem) {
              ...
              if (productImgWrap && visualWrap) {
                ...
                if (shouldFlip) {
                  ...
                  window.hoverDebugger.addLog("DEBUG", `Product → Catalog: Will animate to unhover in ${animationDuration}ms`, activeItem)
                  ...
                  window.hoverDebugger.addLog("DEBUG", "Product → Catalog: Image not visible, using slide-like disappearance", activeItem)
    ...
    views: [
      {
        ...
        afterEnter() {
          ...
          // parallax.refresh()
          reorganizeAndScrollProjectItems.refresh()
        },
      },
    ],
  })
...
document.addEventListener("DOMContentLoaded", function () {
  window.hoverDebugger.addLog("DEBUG", "Page loaded, initializing hover debugger")
})

// Реорганизация каталога и скролл
